...
LOG_FILE="$LOGDIR/train_stsr_resizecond_${STAMP}.log"
ln -sfn "$(basename "$LOG_FILE")" "$LOGDIR/latest_train_stsr_resizecond.log"
...
python -u main.py \
  -t True \
  --base configs/ldm/stsr-x2-resizecond.yaml \
  --logdir "$LOGDIR" \
  --name stsr_x2_resizecond \
