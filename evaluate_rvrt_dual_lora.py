...
import utility
from ldm.models.rvrt_dual_lora_pipeline import RVRTDualLoRAPipeline
...
def load_dual_lora_metadata(ckpt_path):
    state = torch.load(ckpt_path, map_location="cpu")
    return state.get("dual_lora_metadata", {})
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate RVRT + Dual-LoRA LDMVFI pipeline on triplet folders")
    ...
    parser.add_argument("--pixel_lora_patterns", nargs="*", default=None)
    parser.add_argument("--semantic_lora_patterns", nargs="*", default=None)
    ...
    metadata = load_dual_lora_metadata(args.ldm_ckpt)
    ...
    if "pixel_lora_patterns" in metadata:
        ldm_config.model.params.pixel_lora_patterns = list(metadata["pixel_lora_patterns"])
    if "semantic_lora_patterns" in metadata:
        ldm_config.model.params.semantic_lora_patterns = list(metadata["semantic_lora_patterns"])
    if args.pixel_lora_patterns:
        ldm_config.model.params.pixel_lora_patterns = list(args.pixel_lora_patterns)
    if args.semantic_lora_patterns:
        ldm_config.model.params.semantic_lora_patterns = list(args.semantic_lora_patterns)

    pipeline = RVRTDualLoRAPipeline(
